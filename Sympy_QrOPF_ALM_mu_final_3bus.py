...
def solve_subproblem_qhd(
    ...
    raw_response = qhd_model.qhd_base.optimize(verbose=0)
    if raw_response is None or raw_response.minimizer is None:
        raise RuntimeError("QHD backend did not return a coarse minimizer.")

    x_unit = np.asarray(raw_response.minimizer, dtype=float).reshape(-1)
    x_coarse = np.asarray(qhd_model.affine_transformation(x_unit), dtype=float).reshape(-1)
    if x_coarse.size != len(variable_list):
        raise ValueError(
            f"QHD coarse minimizer length mismatch: expected {len(variable_list)}, "
            f"got {x_coarse.size}"
        )

    x_coarse = _clip_to_bounds(x_coarse, var_bound_list)
    print(
        f"[QHD] coarse minimizer returned from backend: len={x_coarse.size}, "
        f"unit_min={np.min(x_unit):.6g}, unit_max={np.max(x_unit):.6g}",
        flush=True,
    )
    return x_coarse
...
def qropf_flow_residual_summary(model: SympyACOPFModel, h_val):
    h_val = np.asarray(h_val, dtype=float).reshape(-1)
    block_slices = model._get_constraint_block_slices()

    summary = {}
    for name in ("P_flow", "Q_flow", "Ssq"):
        start, end, _ = block_slices[name]
        summary[name] = float(np.max(np.abs(h_val[start:end]))) if end > start else 0.0
    summary["PQ_flow"] = max(summary["P_flow"], summary["Q_flow"])
    return summary


def qropf_rank_residual_summary(model: SympyACOPFModel, x_vec):
    vals = model._unpack_x(x_vec)
    wdiag = (
        np.asarray(vals["Wii_RR"], dtype=float)
        * np.asarray(vals["Wii_II"], dtype=float)
        - np.asarray(vals["Wii_RI"], dtype=float) ** 2
    )
    vsq = (
        np.asarray(vals["V_sq"], dtype=float)
        - np.asarray(vals["Wii_RR"], dtype=float)
        - np.asarray(vals["Wii_II"], dtype=float)
    )

    wcross = []
    for p, (i, j) in enumerate(model.cross_pairs):
        wij_real = vals["Wij_RR"][p] + vals["Wij_II"][p]
        wij_imag = vals["Wij_RI"][p] - vals["Wij_IR"][p]
        wcross.append(wij_real ** 2 + wij_imag ** 2 - vals["V_sq"][i] * vals["V_sq"][j])
    wcross = np.asarray(wcross, dtype=float)

    return {
        "Wdiag": float(np.max(np.abs(wdiag))) if wdiag.size else 0.0,
        "Vsq": float(np.max(np.abs(vsq))) if vsq.size else 0.0,
        "Wcross": float(np.max(np.abs(wcross))) if wcross.size else 0.0,
    }


def append_exception_to_log(log_file: str, context: str) -> None:
    with open(log_file, "a", encoding="utf-8") as f:
        f.write("\n")
        f.write("=" * 120 + "\n")
        f.write(f"Exception during {context}\n")
        f.write(traceback.format_exc())
        f.write("\n")


def append_status_to_log(log_file: str, message: str) -> None:
...
def save_objective_plot(
    ...
    history_path = Path(log_folder) / f"{Path(log_file).stem}-Obj.csv"
    with open(history_path, "w", encoding="utf-8") as f:
        f.write("iteration,objective\n")
        for item in valid_history:
            f.write(f"{item['iter']},{item['objective']:.16g}\n")
    print("Objective history CSV:", history_path)
    if show:
        print("[plot] PNG plotting is disabled in this environment; use the CSV history instead.")
...
def run_linear_alm(model: SympyACOPFModel, config: SolverConfig):
    ...
    for k in range(config.max_outer):
        ...
        if config.option == 1:
            append_status_to_log(log_file, f"iteration {k}: starting QHD subproblem")
            ...
            print(
                f"[coarse:QHD] raw minimizer received: len={len(x_coarse)}, "
                f"expected={len(model.variable_list)}",
                flush=True,
            )
            append_status_to_log(log_file, f"iteration {k}: QHD coarse minimizer returned")
            allow_refine = bool(config.qhd_refine)
            try:
                print("[coarse:QHD] evaluating raw lifted constraints", flush=True)
                raw_qhd_h_val = h_func(x_coarse)
                raw_flow_summary = qropf_flow_residual_summary(model, raw_qhd_h_val)
                raw_rank_summary = qropf_rank_residual_summary(model, x_coarse)
                print("[coarse:QHD] projecting P/Q/S branch variables from lifted W", flush=True)
                x_coarse = model.project_branch_flows_from_lifted(x_coarse)
                projected_qhd_h_val = h_func(x_coarse)
                projected_flow_summary = qropf_flow_residual_summary(model, projected_qhd_h_val)
                projected_rank_summary = qropf_rank_residual_summary(model, x_coarse)
                print("[coarse:QHD] building rank-1 lifted start for TNC_orig", flush=True)
                x_refine_start = model.project_rank1_lifted_from_lifted(x_coarse)
                rank1_h_val = h_func(x_refine_start)
                rank1_flow_summary = qropf_flow_residual_summary(model, rank1_h_val)
                rank1_rank_summary = qropf_rank_residual_summary(model, x_refine_start)
                print(
                    "[coarse:QHD W-flow projection] "
                    f"max |P/Q flow def| {raw_flow_summary['PQ_flow']:.3e} -> "
                    f"{projected_flow_summary['PQ_flow']:.3e}; "
                    f"max |Ssq def| {raw_flow_summary['Ssq']:.3e} -> "
                    f"{projected_flow_summary['Ssq']:.3e}",
                    flush=True,
                )
                print(
                    "[coarse:QHD rank1 projection for refine] "
                    f"max |Wdiag| {raw_rank_summary['Wdiag']:.3e} -> "
                    f"{rank1_rank_summary['Wdiag']:.3e}; "
                    f"max |Vsq| {projected_rank_summary['Vsq']:.3e} -> "
                    f"{rank1_rank_summary['Vsq']:.3e}; "
                    f"max |P/Q flow def| {rank1_flow_summary['PQ_flow']:.3e}",
                    flush=True,
                )
            except Exception:
                append_exception_to_log(log_file, f"QHD coarse projection at iteration {k}")
                raise
        else:
            append_status_to_log(log_file, f"iteration {k}: starting Gurobi subproblem")
            x_coarse = solve_subproblem_gurobi(lagrangian, variable_list, var_bound_list)
            x_refine_start = x_coarse
            append_status_to_log(log_file, f"iteration {k}: Gurobi coarse minimizer returned")
        ...
        print(f"[coarse:ALM] max |h(x)| = {coarse_max_abs_h:.6e}", flush=True)
        print(f"[coarse:ALM] ||h(x)|| = {coarse_norm_h:.6e}", flush=True)
        print(f"[coarse:ALM] objective = {coarse_objective_value:.9g}", flush=True)
        ...
        log_file = PrintQHDACOPFResults(
            ...
            iteration=k,
            folder=config.log_folder,
            print_to_console=config.print_to_console and config.option == 1,
            console_format="qr",
            rho=rho_used,
            alpha=alpha_used,
            h_x=coarse_h_val,
        ...
        append_status_to_log(
            log_file,
            f"iteration {k}: coarse solution logged; starting refine {canonical_refine_method(config.refine_method)}",
        )
        ...
        try:
            ...
            append_exception_to_log(log_file, f"refine at iteration {k}")
            raise
        append_status_to_log(log_file, f"iteration {k}: refine completed")
