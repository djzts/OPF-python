...
from opf_beam_selection import candidate_quality_key, select_ranked_diverse_candidates
...
class SolverConfig:
    ...
    qhd_solver: str = "simbi"  # simbi / openjij / gurobi / ibmq
    ...
    # qhd_solver="ibmq": QAOA via Qiskit/Aer (or real IBM Quantum hardware
    # when ibmq_use_simulator=False). See qhdopt/backend/ibmq_backend.py's
    # module docstring before changing these -- ibmq_resolution_bits means
    # BITS per variable (2**ibmq_resolution_bits discretization levels),
    # not grid points like simbi_resolution.
    ibmq_resolution_bits: int = 2
    ibmq_shots: int = 4096
    ibmq_reps: int = 1
    ibmq_optimizer: str = "COBYLA"
    ibmq_maxiter: int = 15
    ibmq_seed: int | None = 42
    ibmq_use_simulator: bool = True
    ibmq_simulator_method: str = "statevector"
    ibmq_channel: str | None = None
    ibmq_instance: str | None = None
    ibmq_token_env_var: str = "IBM_QUANTUM_TOKEN"
    ibmq_backend_name: str | None = None
    ibmq_resilience_level: int = 1
    ibmq_max_qubits: int = 30
    ibmq_consistency_tol: float | None = None
    ...
    beam_refine_limit: int = 3
    beam_merit_penalty: float = 100.0
    beam_residual_band_width: float = 1e-2
    beam_min_normalized_distance: float = 2e-2
...
def validate_config(config: SolverConfig) -> None:
    ...
    if config.qhd_solver not in {"simbi", "openjij", "gurobi", "ibmq"}:
        raise ValueError("qhd_solver must be 'simbi', 'openjij', 'gurobi', or 'ibmq'.")
    ...
    if config.beam_refine_limit <= 0:
        raise ValueError("beam_refine_limit must be positive.")
    if config.beam_merit_penalty < 0.0:
        raise ValueError("beam_merit_penalty must be nonnegative.")
    if config.beam_residual_band_width <= 0.0:
        raise ValueError("beam_residual_band_width must be positive.")
    if config.beam_min_normalized_distance < 0.0:
        raise ValueError("beam_min_normalized_distance must be nonnegative.")
...
def solve_subproblem_qhd(
    ...
    elif config.qhd_solver == "ibmq":
        qhd_model.ibmq_setup(
            resolution=config.ibmq_resolution_bits,
            shots=config.ibmq_shots,
            reps=config.ibmq_reps,
            optimizer=config.ibmq_optimizer,
            maxiter=config.ibmq_maxiter,
            seed=config.ibmq_seed,
            use_simulator=config.ibmq_use_simulator,
            simulator_method=config.ibmq_simulator_method,
            channel=config.ibmq_channel,
            instance=config.ibmq_instance,
            token_env_var=config.ibmq_token_env_var,
            backend_name=config.ibmq_backend_name,
            resilience_level=config.ibmq_resilience_level,
            max_qubits=config.ibmq_max_qubits,
            consistency_tol=config.ibmq_consistency_tol,
            post_processing_method=qhd_post_processing_method,
        )
    else:
...
def run_coarse_beam_search_alm(model: SympyACOPFModel, config: SolverConfig):
    ...
    log_step(
        ...
        f"beam_refine_limit={config.beam_refine_limit}, "
        f"beam_merit_penalty={config.beam_merit_penalty:.12g}, "
        f"beam_residual_band_width={config.beam_residual_band_width:.12g}, "
        f"beam_min_normalized_distance={config.beam_min_normalized_distance:.12g}, "
    ...
    for k in range(config.max_outer):
        ...
        for energy_rank, item in enumerate(selected, start=1):
            ...
            coarse_h = np.asarray(h_func(item["coarse_x"]), dtype=float).reshape(-1)
            item["objective"] = item["coarse_objective"]
            item["norm_h"] = float(np.linalg.norm(coarse_h))
            _, coarse_check_flag = model.check_constraints(item["coarse_x"])
            item["check_flag"] = bool(coarse_check_flag)
            item["refined"] = False
            item["refined_objective"] = None

        quality_key = lambda item: candidate_quality_key(
            item,
            feasibility_tol=config.tol,
            residual_band_width=config.beam_residual_band_width,
            merit_penalty=config.beam_merit_penalty,
        )
        ...
        if config.beam_refine_candidates:
            refine_method = canonical_refine_method(config.refine_method)
            print(
                f"[beam refine] refining "
                f"{min(config.beam_refine_limit, len(selected))}/{len(selected)} "
                f"coarse-ranked candidates with {refine_method}"
            )
            log_step(
                f"beam_refine_start iter={k}, "
                f"candidates={min(config.beam_refine_limit, len(selected))}, "
                f"pool={len(selected)}, method={refine_method}"
            )
            for item in selected[: min(config.beam_refine_limit, len(selected))]:
        ...
        active_candidates = select_ranked_diverse_candidates(
            selected,
            limit=active_candidate_count,
            key=quality_key,
            bounds=original_var_bound_list,
            min_distance=config.beam_min_normalized_distance,
            ignored_indices=fixed_bound_indices,
        )
        ...
        print(
            ...
            f"active merit/diverse top-M={len(active_candidates)}, "
        ...
        best_candidate = {
            ...
            "norm_h": best_record["metric"],
            "check_flag": best_record["feasible"],
        }
        candidate_is_better = (
            best_record["x"] is None or quality_key(evaluation) < quality_key(best_candidate)
        )
